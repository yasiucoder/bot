    ...
    def gather(self, selection=1, disabled_units=[], advanced_gather=True):
        ...
        for option in reversed(sorted(village_data['options'].keys())):
            ...
            if int(option) <= selection and not village_data['options'][option]['is_locked'] and not village_data['options'][option]['scavenging_squad'] != None:
                available_selections.append(int(option))
        ...
        carry_per_selection = total_carry // len(available_selections)

        if selection >= 5:  # If gather_selection is set to 5 or higher
            for selection in available_selections:
                ...
                for item in can_use:
                    ...
                    if item in disabled_units:
                        continue
                    if item in troops and int(troops[item]) > 0:
                        carry_for_unit = min(carry_per_selection // int(carry), int(troops[item]))
                        payload["squad_requests[0][candidate_squad][unit_counts][%s]" % item] = str(carry_for_unit)

                payload["squad_requests[0][candidate_squad][carry_max]"] = str(carry_per_selection)

                if carry_per_selection > 0:
