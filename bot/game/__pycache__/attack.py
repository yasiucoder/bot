...
class AttackManager:
    ...
    def has_high_wall(self, village_id):
        # Retrieve the report for the village from the cache
        village_report = ReportCache.get_cache(village_id)
        if village_report and 'wall_level' in village_report:
            return village_report['wall_level'] > 1
        return False

    def was_attack_lost(self, village_id):
        # Retrieve the report for the village from the cache
        village_report = ReportCache.get_cache(village_id)
        if village_report and 'units_losses' in village_report:
            return bool(village_report['units_losses'])
        return False
    ...
    def send_farm(self, target, template):
        ...
        if not missing:
            ...
            if cached:
                # Calculate the total number of units sent in the attack
                units_sent = sum(template.values())
                self.logger.debug(f"Attacking target {target['id']} with {units_sent} units.")
    ...
    def get_targets(self):
        output = []
        my_village = self.map.villages.get(self.village_id, None)

        for vid, village in self.map.villages.items():
            if village["owner"] != "0" and vid not in self.extra_farm:
                if vid not in self.ignored:
                    self.logger.debug(
                        f"Ignoring village {vid} because it is player owned. Add to additional_farms to auto attack."
                    )
                    self.ignored.append(vid)
                continue

            if "points" in village:
                if village["points"] >= self.farm_maxpoints:
                    if vid not in self.ignored:
                        self.logger.debug(
                            f"Ignoring village {vid} because points {village['points']} exceeds limit {self.farm_maxpoints}."
                        )
                        self.ignored.append(vid)
                    continue
                ...
                if village["points"] <= self.farm_minpoints:
                    if vid not in self.ignored:
                        self.logger.debug(
                            f"Ignoring village {vid} because points {village['points']} is below limit {self.farm_minpoints}."
                        )
                        self.ignored.append(vid)
                    continue

                if my_village and village["points"] >= my_village.get("points", 0) and not self.target_high_points:
                    if vid not in self.ignored:
                        self.logger.debug(
                            f"Ignoring village {vid} because of higher points {my_village.get('points', 0)} -> {village['points']}."
                        )
                        self.ignored.append(vid)
                    continue
            ...
            if distance > self.farm_radius:
                if vid not in self.ignored:
                    self.logger.debug(
                        f"Village {vid} will be ignored because it is too far away: distance is {distance}, max is {self.farm_radius}."
                    )
                    self.ignored.append(vid)
                continue

            if vid in self.ignored:
                self.logger.debug(f"Removed {vid} from farm ignore list.")
                self.ignored.remove(vid)

            output.append([village, distance])

        self.logger.info(
            f"Farm targets: {len(output)} Ignored targets: {len(self.ignored)}"
        )
    ...
    def scout(self, vid):
        if (
            ...
            or int(self.troopmanager.troops["spy"]) < 1
        ):
            self.logger.debug(
