...
class BuildingManager:
    ...
    def get_next_building_action(self, index=0):
        if not self.queue or index >= len(self.queue):
            self.logger.debug("No more items in the queue or index out of range.")
            return False
        ...
        if current_level >= check["max_level"]:
            self.logger.info("Skipping %s because it's already at the maximum level" % entry)
            self.queue.pop(index)
            return self.get_next_building_action(index)
        ...
        if check["can_build"] and self.has_enough(check) and "build_link" in check:
            ...
            # Trigger function again because the game state is changed
            self.costs = self.create_update_links(self.costs)
