    ...
    def get_url(self, url, headers=None):
        ...
        try:
            ...
            if 'data-bot-protect="forced"' in res.text:
                ...
                # Show a notification
                notification_title = "Bot Protection Detected"
                notification_text = "Bot protection hit! Waiting 5 minutes before retrying."
                notification.notify(title=notification_title, message=notification_text, app_icon=None, timeout=10)
                
                # Report the bot protection event
                self.reporter.report(0, "TWB_RECAPTCHA", "Bot protection detected, waiting 5 minutes before retrying")
                
                # Wait for 5 minutes before retrying the request
                time.sleep(1 * 60)
