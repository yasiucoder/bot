        ...
        for farm in attacks:
            ...
            for rep in reports:
                if reports[rep]["dest"] == farm and reports[rep]["type"] == "attack":
                    ...
                    if "units_losses" in reports[rep]["extra"]:
            ...
            if verbose:
                logger.info(
                    "%sFarm village %s attacked %d times - Total loot: %s - Total units lost: %s (%s) - Total units sent: %s",
                    perf, farm, len(num_attack), str(loot), str(total_loss_count), str(percentage_lost), str(total_sent_count)
                )
            if len(num_attack):
                total = 0
                for k in loot:
